...
| --- | --- | --- |
| PriceTrace | `ingest_verified_purchase_price_observation_v1(p_idempotency_key text, p_purchase jsonb)`; `purchase-price-observation.v4` / `purchase-price.v4` | `/rest/v1/rpc/ingest_verified_purchase_price_observation_v1` |
| CashOS | `finance_ingest_transaction_v4`의 flat `p_*` 인자; `cashos.transaction-ingest.v4` | `/rest/v1/rpc/finance_ingest_transaction_v4` |
...
PriceTrace payload는 중첩된 `platform`, nullable `seller`, `order`, `payment`,
`items` shape을 그대로 사용한다. CashOS payload는 다음 flat 인자를 그대로
사용한다.
...
line에 `product_client_key`가 있으면 retail record, 없고 line이 있으면 restaurant
record, line이 없으면 payment-only record로 분류한다. seller가 없더라도 payment-only
CashOS transaction은 가능하다. 날짜나 시각을 사진 촬영·주문 화면 생성 시각·결제 시각에서
자동 대입하지 않는다. 모르는 금액·할인·단가는 `null`로 남긴다.
...
| 조건 | projection | 동작 |
| --- | --- | --- |
| retail/restaurant line + 날짜 + 필요한 price fact | `pricetrace_price_observation` | PriceTrace purchase V4 RPC; seller/authority/product 조건이 충족되지 않으면 PriceTrace observation을 만들지 않는다 |
| grand/paid total + ordered 또는 paid date | `cashos_transaction` | CashOS transaction ingest V4; payment-only도 허용 |
...
각 record는 PriceTrace와 CashOS에 독립된 idempotency/fingerprint를 사용한다. 여러
record가 있는 envelope에서도 CashOS document/idempotency key가 충돌하지 않도록
record client key를 derived key에 포함한다. 서버가 반환한 id만 projection metadata로
보존한다.
...
- V4 strict root는 `schema_version=yeonsik-ocr.v4`, `mode=purchase`,
  `purchase_records`를 요구하고 receipt, legacy `price_observations`, nutrition,
  consumption, links와 혼합하지 않는다.
- V4 source type은 `order_history`, `payment_history`다. history evidence의
  attachment ID는 선언된 동일 type source file을 가리켜야 한다.
...
앞의 네 compile/unit/desktop 검증은 통과했다. 마지막 Android command는 APK와
instrumented test compile까지 수행했지만 현재 `adb devices`에 연결된 device가 없어
