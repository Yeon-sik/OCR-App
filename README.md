...
- Gemini API 키는 교정 화면에서 입력받아 Android Keystore의 AES-GCM 키로 암호화해 기기에 저장합니다. APK·Gradle·Git에는 넣지 않고 현재 키를 화면에 다시 표시하지 않습니다. 직접 API를 호출하는 모바일 앱은 production secret boundary가 아니므로 현재 방식은 개인 사용 범위입니다.
...
1. 저장 세션 목록에서 촬영 또는 갤러리를 시작합니다.
2. ML Kit가 보정한 이미지를 앱 전용 저장소에 보존하고 순서를 확인합니다.
3. 필요한 경우 같은 영수증 구간을 더 추가합니다.
4. OCR을 실행하고 필드와 각 행을 원본/bounding box와 대조합니다. 확인이 필요한 행만 걸러 보거나, 놓친 행을 추가하고 잘못 인식된 행을 삭제할 수 있습니다.
5. 필요하면 `Gemini 교정 제안` 화면에서 본인 API 키를 저장하고 후보를 요청한 뒤, 근거를 보고 필요한 후보만 개별 적용합니다. 이 단계는 선택 사항입니다.
6. 합계와 필수 조건을 확인합니다. 차액이 남으면 제시된 원인 후보를 원본과 대조해 적용하거나, 불일치가 의도된 경우 구체적인 검수 사유를 기록합니다.
7. 초안 JSON은 언제든 앱 내부에 보존할 수 있습니다.
8. 모든 차단 조건을 통과한 경우에만 `user_verified`로 확정하고 로컬 저장/공유합니다.
...
| Gemini model | `gemini-3.5-flash-lite` | 기존 OCR 상품 행의 구조화 교정 후보만 생성; 사용자 승인 전에는 초안을 바꾸지 않음 |
