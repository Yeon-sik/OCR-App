...
data class ReviewViewModel(
    ...
    companion object {
        ...
        fun fromCanonical(
            ...
            val destinationBadges = destinationBadges(envelope, session)
            ...
            val boundIds = evidence.filter(LocalEvidence::fileReadable).map(LocalEvidence::attachmentId).toSet()
            val sourceFiles = if (evidence.isEmpty()) emptyMap() else declaredSourceFiles.filterKeys(boundIds::contains)
            val rows = buildList {
                envelope.receipt?.let { receipt ->
                    add("판매처", "판매처명", receipt.merchant.name, evidenceFor(sourceFiles, emptyList(), envelope.source.userText), destinationBadges)
                    add("판매처", "지점명", receipt.merchant.branchName, evidenceFor(sourceFiles, emptyList(), envelope.source.userText), destinationBadges)
                    add("문서", "구매일", receipt.document.issuedOn, receiptEvidence(sourceFiles), destinationBadges)
                    add("문서", "통화", receipt.document.currency, receiptEvidence(sourceFiles), destinationBadges)
                    add("금액", "최종 결제금액", receipt.totals.grandTotalAmountMinor?.let { "$it ${receipt.document.currency.orEmpty()}" }, receiptEvidence(sourceFiles), destinationBadges)
                ...
                envelope.merchantCandidate?.let { candidate ->
                    add("판매처 후보", "판매처명", candidate.name, evidenceFor(sourceFiles, candidate.sourceAttachmentIds, envelope.source.userText), destinationBadges)
                    add("판매처 후보", "지점명", candidate.branchName, evidenceFor(sourceFiles, candidate.sourceAttachmentIds, envelope.source.userText), destinationBadges)
                    add("판매처 후보", "주소", candidate.address, evidenceFor(sourceFiles, candidate.sourceAttachmentIds, envelope.source.userText), destinationBadges)
                }
                ...
                envelope.purchaseRecords.forEach { purchase -> addPurchaseRows(this, purchase, sourceFiles, envelope, destinationBadges) }
                envelope.nutrition.forEach { nutrition -> addNutritionRow(this, nutrition, sourceFiles, envelope, destinationBadges) }
        ...
        fun fromReceipt(receipt: ReceiptV2, verified: Boolean): ReviewViewModel {
            ...
            val rows = buildList {
                add(ReviewRow("merchant", "판매처", "판매처명", receipt.merchant.name.orEmpty(), evidence, all))
                add(ReviewRow("date", "문서", "구매일", receipt.document.issuedOn.orEmpty(), evidence, all))
                receipt.lineItems.forEach { line ->
                    add(ReviewRow(line.id, "상품", line.description ?: "이름 미확인", line.netAmountMinor?.let { "$it ${receipt.document.currency.orEmpty()}" }.orEmpty(), evidence, all, line.confidence.name))
                }
                add(ReviewRow("total", "금액", "최종 결제금액", receipt.totals.grandTotalAmountMinor?.toString().orEmpty(), evidence, all))
            }.filter { it.value.isNotBlank() }
            return ReviewViewModel(receipt.schemaVersion, rows, all)
        }

        private fun destinationBadges(envelope: YeonsikOcrEnvelope, session: IngestionSession?): List<ReviewDestinationBadge> {
            val plan = CanonicalProjectionPlanner.plan(envelope)
            return ReviewDestination.entries.map { destination ->
                val projections = projectionsFor(destination)
                val eligible = projections.filter { it in plan.eligible }
                val actual = session?.projections.orEmpty().filter { it.projection in projections }
                val status = when {
                    eligible.isNotEmpty() && actual.any { it.status == ProjectionStatus.BLOCKED } -> ReviewDestinationStatus.CONDITION_UNMET
                    eligible.isNotEmpty() -> ReviewDestinationStatus.PLANNED
                    actual.any { it.status == ProjectionStatus.BLOCKED } -> ReviewDestinationStatus.CONDITION_UNMET
                    else -> ReviewDestinationStatus.NOT_APPLICABLE
                }
        ...
        private fun addPurchaseRows(
            ...
            rows.add(ReviewRow("purchase:${purchase.clientKey}", "구매", purchase.platform, listOfNotNull(purchase.status.wireValue, purchase.totals.cashOsAmountKrw?.let { "$it KRW" }).joinToString(" · "), evidence, destinations, percent(purchase.confidence), buildList {
                purchase.seller?.let { add(ReviewDetail("판매처", it)) }
                purchase.orderReference?.let { add(ReviewDetail("주문번호", it)) }
                purchase.payment?.method?.let { add(ReviewDetail("결제수단", it)) }
                purchase.payment?.status?.let { add(ReviewDetail("결제상태", it)) }
            }))
        ...
        private fun addNutritionRow(
            ...
            val nutritionEvidenceIds = sourceFiles.values
                .filter { it.type in setOf(SourceAttachmentType.NUTRITION_LABEL, SourceAttachmentType.PRODUCT_PHOTO, SourceAttachmentType.FOOD_PHOTO, SourceAttachmentType.MENU_PHOTO) }
                .map(SourceAttachment::id)
            rows.add(ReviewRow("nutrition:${nutrition.clientKey}", "영양", name, summary, evidenceFor(sourceFiles, nutritionEvidenceIds, envelope.source.userText), destinations, confidence?.let(::percent), details))
        }
        ...
        private fun SourceAttachmentType.toEvidenceKind(): ReviewEvidenceKind = when (this) {
            ...
            SourceAttachmentType.PRODUCT_PHOTO, SourceAttachmentType.NUTRITION_LABEL -> ReviewEvidenceKind.PRODUCT_PHOTO
